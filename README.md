...
## Demo login (full sample library)
...
This account owns **6 seeded meetings** (transcripts, summaries, topics, action items). After deploying the backend, open `https://YOUR-API/api/seed` once if the library is empty.

New signups create a real account with an empty meeting list until they create meetings.

---

## Assumptions

- **Authentication** — email/password accounts stored in SQLite; session tokens via `Authorization: Bearer`. Demo account above is pre-seeded.
...
- **CORS** — localhost for development; set `FRONTEND_ORIGINS` for production (e.g. Vercel URL).
